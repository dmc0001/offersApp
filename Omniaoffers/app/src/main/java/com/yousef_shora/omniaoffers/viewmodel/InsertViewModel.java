    ...
    public void insert_data(ContentResolver contentResolver) {
        try {
            if (imageUriMutableLiveData.getValue() != null) {
                ...
                byte[] compressed_image = Utils.convert_bitmap_to_byte_Array(current_bitmap, current_bitmap.getWidth(), current_bitmap.getHeight());

                int image_size = Utils.get_bitmap_size(current_bitmap);

                Log.v("image_size", String.valueOf(image_size));

                if (image_size > 500000)
                    compressed_image = Utils.compress_image(imageUriMutableLiveData.getValue(), contentResolver);

                repo.upload_image(compressed_image, offer_to_add.getId(), uploaded -> {
                    if (uploaded) {
